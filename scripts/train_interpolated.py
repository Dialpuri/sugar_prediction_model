...
def sample_generator(dataset: str = "train"):
    ...
    while True:
        for candidate in df_np:
            ...
            no_sugar_path = os.path.join(
                param.dataset_base_dir, pdb_code, f"{pdb_code}{names.no_sugar_file}.map"
            )

            density_map = gemmi.read_ccp4_map(density_path).grid
            sugar_map = gemmi.read_ccp4_map(sugar_path).grid
            phosphate_map = gemmi.read_ccp4_map(phosphate_path).grid
            base_map = gemmi.read_ccp4_map(base_path).grid
            no_sugar_map = gemmi.read_ccp4_map(no_sugar_path).grid
            ...
            sugar_array = np.array(
                sugar_map.get_subarray(
                    start=[X, Y, Z], shape=[param.shape, param.shape, param.shape]
                )
            )
            # phosphate_array = np.array(
            ...
            #     )
            # )
            # no_sugar_array = np.array(
            #     no_sugar_map.get_subarray(
            #         start=[X, Y, Z], shape=[param.shape, param.shape, param.shape]
            #     )
            # )

            density_yield = density_array.reshape(
...
def train():
    ...
    weight_path = "models/interpolated_model.best.hdf5"
    ...
    train_dataset = train_dataset.repeat(EPOCHS).batch(
        batch_size=BATCH_SIZE)

    test_dataset = test_dataset.repeat(EPOCHS).batch(
        batch_size=BATCH_SIZE)
    ...
    model.save("models/interpolated_model")
